...
require "../../../controllers/EpisodeController.php";
        ...
        if(empty($_POST['name']) || empty($_POST['show_id']) ){
            echo "<script>alert('one of the inputs are empty')</script>";
        }
        else
        {   
            $name=$_POST['name'];
            ...
            $episode= new Episode($name, $show_id, $thumbnail,$video);
            
            $epctr=new EpisodeController();
            $epctr->insertEpisode($episode);

            if(move_uploaded_file($_FILES['thumbnail']['tmp_name'], $dir_thumbnail) && move_uploaded_file($_FILES['video']['tmp_name'], $dir_video))
            {
